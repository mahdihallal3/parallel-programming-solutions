...
void *producer (void *parg)
{
  ...
  while (1) {
     ...
     */
     ...
     pthread_mutex_lock (fifo->lock); //mutex lock for synchronizing the communication between the pros and cons
     
    while (fifo->full && *total_produced != WORK_MAX) {
        
      printf ("prod %d:  FULL.\n", my_tid);
      pthread_cond_wait(fifo->notFull, fifo->lock); //if buffer full then wait for signal from consumers
...
void *consumer (void *carg)
{
  ...
  while (1) {
     ...
     */
     ...
     pthread_mutex_lock (fifo->lock); //mutex lock for synchronizing the communication between pros and cons
     
    while (fifo->empty && *total_consumed != WORK_MAX) {
      printf ("con %d:   EMPTY.\n", my_tid);
      pthread_cond_wait (fifo->notEmpty, fifo->lock); //if buffer is empty then wait for a signal from producers
    }
  ...
  printf("con %d:   exited\n", my_tid);
  //a signal from a consumer to other consumers to make sure they all terminate
...
int main (int argc, char *argv[])
{
  ...
  pros = rand()%16; //generates a random number of producers between 1 and 16
  cons = rand()%16; //generates a random number of consumers between 1 and 16
  ...
  printf("\nTime: %lu", (stop.tv_sec-start.tv_sec)*1000000 + stop.tv_usec - start.tv_usec); //displaying time
